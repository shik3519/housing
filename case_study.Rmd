...
#removing outlier observations: 84 observations
df.new <- df1[-d.outlier,]

#predictor transformation

x.skew <- c("MSSubClass","LotArea","BsmtFinSF1","BsmtFinSF2","BsmtUnfSF","TotalBsmtSF","X1stFlrSF","X2ndFlrSF"  ,"LowQualFinSF","GrLivArea","BsmtHalfBath","KitchenAbvGr","WoodDeckSF","OpenPorchSF","EnclosedPorch","X3SsnPorch"   
,"ScreenPorch","PoolArea","MiscVal")
tmp1 <- df.new[,x.skew]
tmp1[tmp1==0] <- 1
tmp1 <- as.data.frame(sapply(tmp1,log))
tmp2 <- varremove(df.new,x.skew)
df.new1 <- cbind(tmp1,tmp2)

#creating test and trainig datasets
set.seed(1)
train <- sample(1:nrow(df.new1), nrow(df.new1)*.8)
test <- (-train)
df.new1$Yrdiff <- df.new1$YrSold - df.new1$YearBuilt
df.new1$bsmtarea <- df.new1$TotalBsmtSF+df.new1$X1stFlrSF+df.new1$X2ndFlrSF
y <- df.new1$y
...
#log transformation
df.train$y <- log(df.train$y)
df.test$y <- log(df.test$y)
...
#300299981
mspe_ols_log <- mean((base_predict - df.test$y)^2)
#0.00958385
